//
// MD5.cpp
// based on MD5C.C - RSA Data Security, Inc., MD5 message-digest algorithm
// see RFC 1321
//
...
#include <memory.h>
#define MD5_memcpy(PD,PS,s) memcpy(PD,PS,s)
#define MD5_memzero(PD,s) memset(PD,0,s)
    ...
    )
{
    ...
    {
        output[ i ] = MD5::UINT32( input[ j ] ) 
            | ( MD5::UINT32( input[ j + 1 ] ) << 8 )
            | ( MD5::UINT32( input[ j + 2 ] ) << 16 )
            | ( MD5::UINT32( input[ j + 3 ] ) << 24 );
        }
    }
    ...
    )
{
    ...
    {
        hash[ j++ ] = hex_digit[ bin_hash[ i ] >> 4 ];
        hash[ j++ ] = hex_digit[ bin_hash[ i ] & 0xF ];
        }
    ...
    )
{
    ...
    //
    MD5_memzero( POINTER( this ), sizeof( *this ) );
    }
    ...
    )
{
    ...
    // Zeroize sensitive information
    //
    MD5_memzero( POINTER( x ), sizeof( x ) );
    }
